...
// Extras
public function addExtra($service_id){
    $page_title = 'formfiletext';//For Layout Inheritance
    $page_name = 'Add addExtra';
    return view('admin.addExtra',compact('page_title','page_name','service_id'));
}
...
public function extras($serivice_id){
    $Extra = DB::table('extras')->where('service_id',$serivice_id)->get();
    $page_title = 'list';
    $page_name = 'Extras';
    return view('admin.extras',compact('page_title','Extra','page_name','serivice_id'));
}

public function editExtras($id){
    $Extra = Extra::find($id);
    $page_title = 'formfiletext';
    $page_name = 'Edit Extra';
    return view('admin.editExtras',compact('page_title','Extra','page_name'));
}
...
public function addIndustries(){
    $page_title = 'formfiletext';//For Layout Inheritance
    $page_name = 'Add New Industries';
    return view('admin.addIndustries',compact('page_title','page_name'));
}
...
public function add_Industries(Request $request){
    ...
    $Industry->title = $request->name;
    $Industry->content = $request->content;
    $Industry->image = $image_one;
   
    $Industry->save();
  
    Session::flash('message', "Industry Has Been Added");
    return Redirect::back();
}
...
public function industries(){
    ...
    $page_title = 'list';
    $page_name = 'Industry';
    return view('admin.industries',compact('page_title','Industry','page_name'));
}

public function editIndustries($id){
    $Industry = Industry::find($id);
    $page_title = 'formfiletext';
    $page_name = 'Edit Industries';
    return view('admin.editIndustries',compact('page_title','Industry','page_name'));
}


public function edit_Industries (Request $request, $id){
    $path = 'uploads/industries';
    if(isset($request->image_one)){
        $fileSize = $request->file('image_one')->getClientSize();
            if($fileSize>=1800000){
    ...
    DB::table('industries')->where('id',$id)->update($updateDetails);
    Session::flash('message', "Changes have been saved");
    return Redirect::back();
}
...
public function add_Portfolio(Request $request){
    ...
    $Portfolio->venue = $request->venue;
    $Portfolio->audience = $request->audience;
